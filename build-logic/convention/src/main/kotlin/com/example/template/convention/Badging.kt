...
abstract class CheckBadgingTask : DefaultTask() {
    ...
    @TaskAction
    fun taskAction() {
        assert(generatedBadging.get().asFile.readText() == goldenBadging.get().asFile.readText()) {
            println(
                """Generated badging is different from golden badging! If this change is intended, run ./gradlew ${updateBadgingTaskName.get()}"""
            )
        }
    }
}
